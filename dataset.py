...
class ContentStyleDataset(torch.utils.data.Dataset):
    def __init__(self, content_dir, albums_dir, style_dir, transform=None):
        self.content_dir = content_dir
        self.style_dir = style_dir
        self.transform = transform
        self.content_filenames = [content_dir + "/" + filename for filename in os.listdir(content_dir) if filename.endswith('.jpg') or filename.endswith('.png') or filename.endswith('.jpeg')][:20000]
        ...
        self.content_filenames = self.content_filenames[:30000]
        self.style_filenames = [filename for filename in os.listdir(style_dir) if filename.endswith('.jpg') or filename.endswith('.jpeg') or filename.endswith('.png')][:10000]
